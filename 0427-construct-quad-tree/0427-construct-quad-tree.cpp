...
public:
    ...
    {
        if(len==1) return new Node(grid[x1][y1],true);
        Node *topLeft=solve(grid,x1,y1,len/2);
        Node *topRight=solve(grid,x1,y1+len/2,len/2);
        Node *bottomLeft=solve(grid,x1+len/2,y1,len/2);
        Node *bottomRight=solve(grid,x1+len/2,y1+len/2,len/2);
