...
public:
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        for(int i=0;i<m;i++) q.push({i,0}), vis1[i][0]=1;
        for(int j=0;j<n;j++) q.push({0,j}), vis1[0][j]=1;
        bfs(heights,q,m,n,vis1);
        while(!q.empty()) q.pop();
        for(int i=0;i<m;i++) q.push({i,n-1}), vis2[i][n-1]=1;
        for(int j=0;j<n;j++) q.push({m-1,j}), vis2[m-1][j]=1;
        bfs(heights,q,m,n,vis2);
        vector<vector<int>> res;
        for(int i=0;i<m;i++)
            for(int j=0;j<n;j++)
                if(vis1[i][j]==vis2[i][j] and vis1[i][j]==1) res.push_back({i,j});
        return res;
    }
};